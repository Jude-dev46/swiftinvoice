    ...
    <div className="bg-white w-[98%] md:w-[100%] lg:w-[96%] lg:h-[400px] shadow-md rounded-md">
      ...
      <div className="overflow-y-scroll max-h-[85%]">
        <div className="w-full flex justify-between items-center text-black font-bold px-8 py-4">
          ...
          <p className="w-fit md:w-[20%] text-left">Email</p>
          <p className="w-fit md:w-[20%] text-left">PhoneNo</p>
        </div>
        {DUMMY_CLIENTS.map((client) => (
          <div key={client.clientId}>
            <div className="w-full flex justify-between items-center text-black font-bold px-8 py-4">
              <p className="w-fit md:w-[20%] text-left">{client.clientId}</p>
              <p className="w-fit md:w-[20%] text-left">{client.clientName}</p>
              <p className="w-fit md:w-[20%] text-left">{client.email}</p>
              <p className="w-fit md:w-[20%] text-left">{client.phoneNo}</p>
            </div>
          </div>
        ))}
      </div>
